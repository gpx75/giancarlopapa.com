import type { PersistedCvSuggestion, CvSuggestionStatus } from '~/types/applications';
...
export function useCvSuggestions(applicationId: MaybeRefOrGetter<number>) {
  ...
  async function regenerate() {
    ...
    try {
      ...
      toast.add({ title: 'Suggestions regenerated', color: 'success', icon: 'i-lucide-check' });
    } catch (err: unknown) {
  ...
  async function setStatus(suggestionId: number, status: CvSuggestionStatus, applied_note?: string) {
    try {
      ...
      toast.add({ title: 'Update failed', description: msg, color: 'error', icon: 'i-lucide-triangle-alert' });
    }
  }

  const counters = computed(() => {
    const total = suggestions.value.length;
    const applied = suggestions.value.filter(s => s.status === 'applied').length;
    const dismissed = suggestions.value.filter(s => s.status === 'dismissed').length;
    const pending = total - applied - dismissed;
    return { total, applied, dismissed, pending };
  });

  return { suggestions, loading, generating, refresh, regenerate, setStatus, counters };
}
