...
async function load() {
  ...
  const data = await $fetch(
    `/api/admin/applications/${props.applicationId}/tailored-resume`
  );
...
async function downloadPdf() {
  ...
  try {
    const blob = await $fetch(
      `/api/admin/applications/${props.applicationId}/resume-pdf`,
      { responseType: 'blob' }
    );
    const url = URL.createObjectURL(blob as unknown as Blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `resume-${props.company.toLowerCase()}-tailored.pdf`;
    a.click();
...
<template>
  <div class="rounded-lg border border-default bg-elevated/30">
    ...
    <div v-if="open" class="border-t border-default p-3 space-y-3">
      ...
      <div v-if="jsonError" class="text-xs text-error font-mono">
        {{ jsonError }}
      </div>
