...
const priority = ref<ApplicationPriority | null>(props.application.priority ?? null);
...
watch(() => props.application.id, () => {
  ...
  rationale.value = props.application.workflow.stages.prioritize.rationale ?? '';
});

const PRIORITY_OPTIONS: { value: ApplicationPriority, label: string, color: 'error' | 'warning' | 'neutral' }[] = [
...
async function saveDraft() {
  if (!priority.value) {
    toast.add({ title: 'Pick a priority first', color: 'warning', icon: 'i-lucide-flag' });
    return;
  }
  saving.value = true;
  try {
    ...
    toast.add({ title: 'Priority saved', color: 'success', icon: 'i-lucide-check' });
  } catch (err: unknown) {
...
async function confirm() {
  ...
  if (!rationale.value.trim()) {
    toast.add({ title: 'Rationale required', description: 'A short note keeps prioritisation honest.', color: 'warning', icon: 'i-lucide-pen-line' });
    return;
  }
  ...
  try {
    ...
    await $fetch(`/api/admin/applications/${props.application.id}/workflow`, {
      method: 'POST',
      body: { stage: 'prioritize', action: 'complete', meta: { rationale: rationale.value.trim() } }
    });
    emit('transitioned');
    toast.add({ title: 'Prioritisation locked in', color: 'success', icon: 'i-lucide-check' });
  } catch (err: unknown) {
    const msg = err instanceof Error ? err.message : 'Confirm failed.';
    toast.add({ title: 'Confirm failed', description: msg, color: 'error', icon: 'i-lucide-triangle-alert' });
  } finally {
...
<template>
  <UCard>
      ...
      <div class="flex items-center justify-between gap-3">
        <div>
          ...
          <p class="text-xs text-neutral-500 dark:text-neutral-400 mt-1">
            Decide if this role earns your time. Confirm with a short rationale — no auto-progress.
          </p>
        </div>
        <UBadge
    ...
    <div class="space-y-5">
      ...
      <div>
        <label class="text-xs font-mono text-neutral-500 dark:text-neutral-400 uppercase tracking-wide mb-2 block">
          Priority tier
        </label>
