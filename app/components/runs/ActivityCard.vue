...
const sportLabel = computed(() => {
  const labels = ui.value?.runs.sportLabels ?? {};
  return labels[props.activity.sport_type] ?? props.activity.sport_type;
});
...
<template>
  <UCard :ui="{ root: 'flex flex-col', body: 'p-0' }" variant="subtle">
    <!-- Map -->
    <ClientOnly v-if="activity.summary_polyline">
      <RunsActivityMap :polyline="activity.summary_polyline" />
      <template #fallback>
        <div class="h-36 bg-elevated/50 animate-pulse" />
      </template>
    </ClientOnly>
    <div v-else class="h-36 flex items-center justify-center bg-elevated/50">
      <UIcon name="i-lucide-map-off" class="size-6 text-muted" />
    </div>
    ...
    <div class="px-4 py-3 space-y-3">
      <div>
        <div class="flex items-center gap-2">
          <span class="text-xs font-mono font-medium" :class="sportColor">{{
            sportLabel
          }}</span>
          <span class="text-xs text-muted">·</span>
          <p class="text-xs text-muted font-mono">{{ date }}</p>
        </div>
