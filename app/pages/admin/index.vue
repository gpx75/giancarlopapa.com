...
const { data: topMatches } = await useAsyncData(
...
const pipeline = computed(() => [
  {
    label: 'Saved',
    value: stats.value?.pipeline?.saved ?? 0,
    color: 'text-muted',
    icon: 'i-lucide-bookmark'
  },
  {
    label: 'Applied',
    value: stats.value?.pipeline?.applied ?? 0,
    color: 'text-primary',
    icon: 'i-lucide-send'
  },
  {
    label: 'Interviewing',
    value: stats.value?.pipeline?.interviewing ?? 0,
    color: 'text-info',
    icon: 'i-lucide-users'
  },
  {
    label: 'Offered',
    value: stats.value?.pipeline?.offered ?? 0,
    color: 'text-success',
...
const highMatchSuggestions = computed(
  () => stats.value?.highMatchSuggestions ?? 0
);
  ...
  <UDashboardPanel id="admin-overview">
    ...
    <template #body>
      <div class="flex flex-col gap-6 p-4 max-w-5xl">
        <!-- Pipeline Funnel -->
        <div>
          <p class="text-xs text-muted uppercase tracking-wide mb-3">
          ...
          <UCard :ui="{ body: 'p-4' }">
            <div class="flex items-center gap-1 flex-wrap">
              ...
              >
                <div
                  class="flex flex-col items-center gap-1 px-4 py-2 rounded-lg bg-elevated/60 min-w-[80px] text-center"
                >
                  <UIcon
                    :name="stage.icon"
                    class="size-4"
                    :class="stage.color"
                  />
                  <span class="text-2xl font-bold" :class="stage.color">{{
                    stage.value
                  }}</span>
              ...
              <div class="flex items-start gap-3">
                <div class="relative shrink-0 mt-0.5">
                  <UIcon
                    ...
                    class="size-5"
                    :class="
                      highMatchSuggestions > 0 ? 'text-primary' : 'text-muted'
                    "
                  />
                  <span
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <p
                    ...
                    :class="
                      newSuggestions > 0 ? 'text-foreground' : 'text-muted'
                    "
                  >
                    {{ newSuggestions }}
                  </p>
                  <p class="text-xs text-muted mt-0.5">
                    <span
                      ...
                      >{{ highMatchSuggestions }} high-match</span
                    >
            ...
            <div class="flex items-start gap-3">
              ...
              <div class="flex-1 min-w-0">
                ...
                <p class="text-xs text-muted mt-2">
                  {{ matchDist.total }} of
                  {{ matchDist.totalApplications }} analyzed
                </p>
              </div>
            </div>
          </UCard>
            ...
            <div class="flex items-center gap-3">
              ...
              <div>
                ...
                <p class="text-xs text-muted">
                  {{ stats?.total ?? 0 }} total submissions
                </p>
              </div>
            </div>
          </UCard>
        ...
        <div v-if="topMatches && topMatches.length > 0">
          ...
          <div class="flex flex-col gap-2">
            ...
            >
              <UCard
                :ui="{ body: 'p-3' }"
                class="hover:bg-elevated/60 transition-colors cursor-pointer"
              >
                <div class="flex items-center gap-3">
                  <UBadge
        ...
        <div v-else-if="topMatches !== null">
          <UCard :ui="{ body: 'p-6' }">
            <div
              class="flex flex-col items-center gap-2 text-center text-muted"
            >
