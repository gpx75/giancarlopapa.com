...
const STAGES = ['analyze', 'prioritize', 'cv', 'cover_letter', 'review', 'apply', 'interview_prep'] as const;
...
const headerStatusColor = computed(() => {
  ...
  if (w.current_stage === 'interview_prep' || w.current_stage === 'sent') return 'info';
  return 'info';
});
...
async function confirmDelete() {
  ...
  deleting.value = true;
  try {
    await $fetch(`/api/admin/applications/${application.value.id}`, { method: 'DELETE' });
    toast.add({ title: 'Application deleted', color: 'success', icon: 'i-lucide-trash-2' });
...
<template>
  <UDashboardPanel id="admin-application-canvas">
    <template #header>
      <UDashboardNavbar :title="application?.position ?? 'Application'" :ui="{ right: 'gap-2' }">
        <template #leading>
    ...
    <template #body>
      ...
      <div v-else-if="!application" class="p-6">
        <UAlert color="error" variant="soft" icon="i-lucide-triangle-alert" title="Application not found." />
        <UButton class="mt-3" to="/admin/applications" icon="i-lucide-arrow-left" variant="soft">
      ...
      <div v-else class="p-4 sm:p-6 space-y-4">
        ...
        <div class="flex items-start justify-between gap-3 flex-wrap">
          <div class="min-w-0 space-y-1">
            <div class="flex items-center gap-2 min-w-0">
              <UIcon name="i-lucide-building-2" class="text-primary size-5 shrink-0" />
              <span class="text-xl sm:text-2xl font-semibold text-default truncate">
                {{ application.company }}
              </span>
            </div>
            <div class="flex items-center gap-2 text-sm text-muted font-mono truncate pl-7">
              <span v-if="application.location">{{ application.location }}</span>
              <span v-if="application.location && application.work_model" class="opacity-50">·</span>
              <span v-if="application.work_model">{{ application.work_model }}</span>
              <span v-if="!application.location && !application.work_model" class="italic">No location specified</span>
            </div>
          </div>
          <div class="flex items-center gap-2">
            <UBadge v-if="application.match_rate !== null" color="primary" variant="soft">
        ...
        <AdminJobDescriptionViewer :application="application" @saved="onJdSaved" />
        ...
        <UModal
          ...
          :description="application ? `${application.company} — ${application.position}` : undefined"
        >
          <template #body>
            <p class="text-sm text-muted">
              This soft-deletes the application and all its workflow history. It can be
              restored by clearing <code class="font-mono text-xs">deleted_at</code> in the database.
            </p>
          </template>
          <template #footer>
