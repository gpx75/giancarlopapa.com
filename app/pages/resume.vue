...
const education = computed<ResumeEducation[]>(() => resume.value?.education ?? [])
const skills = computed<ResumeSkill[]>(() => resume.value?.skills ?? [])
const languages = computed<ResumeLanguage[]>(() => resume.value?.languages ?? [])
const interests = computed<ResumeInterest[]>(() => resume.value?.interests ?? [])
...
<template>
  <UContainer class="space-y-12 py-12 lg:space-y-16 lg:py-16">
    <div class="flex items-center justify-between">
      <div>
        <p class="text-xs uppercase tracking-widest text-muted/80">
          Resume
        </p>
