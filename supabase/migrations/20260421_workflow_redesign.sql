...
create index if not exists idx_job_applications_deleted_at
...
create index if not exists idx_job_applications_match_rate
  on public.job_applications (match_rate desc nulls last)
  where deleted_at is null;
...
create index if not exists idx_app_cv_suggestions_app_id
  on public.application_cv_suggestions (application_id, created_at desc);

create index if not exists idx_app_cv_suggestions_run
  on public.application_cv_suggestions (application_id, run_id);
...
create index if not exists idx_app_ref_letters_app_id
  on public.application_reference_letters (application_id);
...
create index if not exists idx_app_workflow_history_app_id
  on public.application_workflow_history (application_id, created_at desc);
...
create index if not exists idx_job_suggestions_dismissed_at
  on public.job_suggestions (dismissed_at)
  where dismissed_at is null;

create index if not exists idx_job_suggestions_status_created
  on public.job_suggestions (status, created_at desc)
  where dismissed_at is null;
...
create unique index if not exists uniq_cover_letter_active_per_app
  on public.cover_letters (application_id)
  where is_active;

create index if not exists idx_cover_letters_app_id
  on public.cover_letters (application_id, created_at desc);
...
update public.job_applications
   set workflow = jsonb_build_object(
     'current_stage', 'sent',
     'stages', jsonb_build_object(
       'analyze',      jsonb_build_object('status', case when match_rate is not null then 'done' else 'pending' end),
       'prioritize',   jsonb_build_object('status', case when priority is not null then 'done' else 'pending' end),
       'cv',           jsonb_build_object('status', case when cv_suggestions is not null then 'done' else 'pending' end, 'applied_count', 0, 'total_count', coalesce(jsonb_array_length(cv_suggestions), 0)),
       'cover_letter', jsonb_build_object('status', 'done', 'version_count', 0),
       'review',       jsonb_build_object('status', 'done', 'checklist', '{}'::jsonb),
       'apply',        jsonb_build_object('status', 'done', 'mode', 'send', 'sent_at', applied_at)
     )
   )
 where status in ('applied', 'interviewing', 'offered', 'accepted', 'rejected', 'withdrawn')
   and (workflow ->> 'current_stage') = 'analyze';

-- Migrate any inline cv_suggestions json into the new table (only once).
insert into public.application_cv_suggestions (application_id, section, issue, suggestion, priority, status)
select
  a.id,
  coalesce(s ->> 'section', 'General'),
  coalesce(s ->> 'issue', ''),
  coalesce(s ->> 'suggestion', ''),
  case when s ->> 'priority' in ('high', 'medium', 'low') then s ->> 'priority' else 'medium' end,
  'pending'
from public.job_applications a,
     jsonb_array_elements(coalesce(a.cv_suggestions, '[]'::jsonb)) s
where jsonb_typeof(a.cv_suggestions) = 'array'
  and not exists (
    select 1 from public.application_cv_suggestions x where x.application_id = a.id
  );
