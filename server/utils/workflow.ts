...
export function defaultWorkflow(): ApplicationWorkflow {
  return {
    ...
    stages: {
      ...
      cover_letter:   { status: 'pending', version_count: 0, active_version_id: null },
      review:         { status: 'pending', checklist: {} },
      apply:          { status: 'pending', mode: null, sent_at: null, exported_at: null },
      interview_prep: { status: 'pending', notes: null, checklist: {}, scheduled_at: null }
    }
  };
}
...
export function normalizeWorkflow(raw: unknown): ApplicationWorkflow {
  ...
  const stages: WorkflowStages = {
    ...
    cover_letter:   { ...base.stages.cover_letter,   ...(inStages.cover_letter ?? {}) },
    review:         { ...base.stages.review,         ...(inStages.review ?? {}), checklist: { ...base.stages.review.checklist, ...(inStages.review?.checklist ?? {}) } },
    apply:          { ...base.stages.apply,          ...(inStages.apply ?? {}) },
    interview_prep: { ...base.stages.interview_prep, ...(inStages.interview_prep ?? {}), checklist: { ...base.stages.interview_prep.checklist, ...(inStages.interview_prep?.checklist ?? {}) } }
  };
...
export function isReviewComplete(checklist: ReviewChecklist | undefined | null): boolean {
...
function setStageStatus(stages: WorkflowStages, stage: WorkflowStage, status: StageStatus): WorkflowStages {
...
export function applyTransition(
  ...
  if (action === 'enter') {
    ...
    if (stage === 'interview_prep') {
      ...
      stages = {
        ...
        interview_prep: {
          ...cur,
          notes: typeof meta?.notes === 'string' ? meta.notes : cur.notes ?? null,
          checklist: (meta?.checklist as InterviewChecklist | undefined) ?? cur.checklist,
          scheduled_at: typeof meta?.scheduled_at === 'string' ? meta.scheduled_at : cur.scheduled_at ?? null
        }
      };
    }
  ...
  if (action === 'complete') {
    if (stage === 'apply') {
      ...
      const mode = (meta?.mode === 'export' || meta?.mode === 'send') ? meta.mode : null;
      const stages: WorkflowStages = {
        ...workflow.stages,
        apply: {
          status: 'done',
          mode,
          sent_at: mode === 'send' ? new Date().toISOString() : workflow.stages.apply.sent_at ?? null,
          exported_at: mode === 'export' ? new Date().toISOString() : workflow.stages.apply.exported_at ?? null
        },
        interview_prep: {
          ...workflow.stages.interview_prep,
          status: workflow.stages.interview_prep.status === 'done' ? 'done' : 'in_progress'
        }
      };
    ...
    if (stage === 'interview_prep') {
      const notes = typeof meta?.notes === 'string' ? meta.notes : workflow.stages.interview_prep.notes ?? null;
      const checklist = (meta?.checklist as InterviewChecklist | undefined) ?? workflow.stages.interview_prep.checklist;
      const scheduled_at = typeof meta?.scheduled_at === 'string' ? meta.scheduled_at : workflow.stages.interview_prep.scheduled_at ?? null;
      const stages: WorkflowStages = {
        ...workflow.stages,
        interview_prep: { ...workflow.stages.interview_prep, status: 'done', notes, checklist, scheduled_at }
      };
      return { current_stage: 'closed', stages };
    }

    if (stage === 'review') {
      const checklist = (meta?.checklist as ReviewChecklist | undefined) ?? workflow.stages.review.checklist;
    ...
    if (stage === 'analyze') {
      stages = {
        ...stages,
        analyze: { ...stages.analyze, status: 'done', last_run_at: new Date().toISOString() }
      };
    }
...
export function refreshStageCounters(
  ...
  if (typeof patch.cv_applied_count === 'number' || typeof patch.cv_total_count === 'number') {
    stages.cv = {
      ...stages.cv,
  ...
  if (typeof patch.cover_letter_version_count === 'number'
      || patch.cover_letter_active_version_id !== undefined) {
    stages.cover_letter = {
      ...stages.cover_letter,
      version_count: patch.cover_letter_version_count ?? stages.cover_letter.version_count,
      active_version_id: patch.cover_letter_active_version_id !== undefined
        ? patch.cover_letter_active_version_id
        : stages.cover_letter.active_version_id ?? null
    };
  }
  if (patch.analyze_last_run_at) {
    stages.analyze = { ...stages.analyze, last_run_at: patch.analyze_last_run_at };
  }
