...
function buildResumeHtml(avatarBase64: string | undefined): string {
  ...
  const resume = resumeJson as typeof resumeJson & {
    ...
    return `
      <div class="work-row">
        ...
        <div class="wcontent">
          ...
          ${job.summary ? `<div class="jsummary">${escHtml(job.summary)}</div>` : ''}
...
<body>
  ...
  ${skills
    .map(
      (g) => `
    <div class="skill-row">
      <span class="skill-label">${escHtml(g.name)}:</span>
      <span class="skill-kw-inline">${g.keywords.map((k) => escHtml(k)).join(', ')}</span>
    </div>`
    )
    .join('')}
  ...
  ${
    ...
    ${projects
      ...
      <div class="project-row">
        ...
        <div class="project-stack">${escHtml(p.stack)}</div>
      </div>`
      )
      .join('')}
  `
