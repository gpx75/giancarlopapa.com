...
export default defineEventHandler(async (event) => {
  ...
  const prior = (priorRes.data ?? []) as Array<{
    id: number;
    status: string;
    suggestion: string;
    section: string;
  ...
  try {
    ...
    console.error(
      '[cv-suggestions] Failed to parse:',
  ...
  if (pendingPrior.length > 0) {
    const supersedeRes = await db
      ...
      .in(
        'id',
        pendingPrior.map((p) => p.id)
      );
    if (supersedeRes.error) {
      console.warn(
        '[cv-suggestions] Failed to supersede prior pending:',
        supersedeRes.error.message
      );
    }
  }
