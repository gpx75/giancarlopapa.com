...
type StageKey = 'analyze' | 'prioritize' | 'cv' | 'cover_letter' | 'review' | 'apply' | 'interview_prep' | 'closed';

const STAGES: StageKey[] = ['analyze', 'prioritize', 'cv', 'cover_letter', 'review', 'apply', 'interview_prep', 'closed'];
...
export default defineEventHandler(async (event) => {
  ...
  const [appsRes, historyRes] = await Promise.all([
    db.from('job_applications')
      .select('id, company, status, source, created_at, applied_at, decided_at, workflow, match_rate')
      .is('deleted_at', null),
    db.from('application_workflow_history')
      .select('application_id, stage, action, created_at')
      .order('created_at', { ascending: true })
  ]);

  if (appsRes.error) throw createError({ statusCode: 500, message: appsRes.error.message });
  if (historyRes.error) throw createError({ statusCode: 500, message: historyRes.error.message });
  ...
  const stageCounts: Record<StageKey, number> = {
    analyze: 0, prioritize: 0, cv: 0, cover_letter: 0, review: 0, apply: 0, interview_prep: 0, closed: 0
  };
  for (const a of apps) {
  ...
  const stageDurations: Record<StageKey, number[]> = {
    analyze: [], prioritize: [], cv: [], cover_letter: [], review: [], apply: [], interview_prep: [], closed: []
  };
  ...
  for (const [, events] of byApp) {
    for (let i = 0; i < events.length; i++) {
      ...
      const next = events.slice(i + 1).find((e) => e.action === 'enter' || e.action === 'complete');
  ...
  const stageAvgMs: Record<StageKey, number | null> = {
    analyze: null, prioritize: null, cv: null, cover_letter: null, review: null, apply: null, interview_prep: null, closed: null
  };
  for (const s of STAGES) {
    const arr = stageDurations[s];
    stageAvgMs[s] = arr.length ? arr.reduce((acc, n) => acc + n, 0) / arr.length : null;
  }
  ...
  const winRate = decided.length ? Math.round((offers / decided.length) * 100) : null;

  // ---- Source effectiveness ------------------------------------------
  // Bucket each app by `source` (free-text on job_applications row);
  // count totals + offers + avg match_rate.
  const sourceMap = new Map<string, { total: number; offers: number; rates: number[] }>();
  for (const a of apps) {
  ...
  const sources = Array.from(sourceMap.entries())
    .map(([source, b]) => ({
      ...
      avg_match: b.rates.length ? Math.round(b.rates.reduce((a, n) => a + n, 0) / b.rates.length) : null
    }))
  ...
  const recent = {
    created: apps.filter((a) => new Date(a.created_at).getTime() >= cutoff).length,
    applied: apps.filter((a) => a.applied_at && new Date(a.applied_at).getTime() >= cutoff).length,
    decided: apps.filter((a) => a.decided_at && new Date(a.decided_at).getTime() >= cutoff).length
  };
